...
#let examify(function) = {
  (name: "", exams: "", body) => {
    let exams = if (type(exams) == array) {
      exams.join(", ")
    } else {exams}
    function(title: [*#name* #"   " _#exams _], body)
  }
}

#let (theorem, corollary, lemma, definition, approach, notation, note, task) = (theorem, corollary, lemma, definition, approach, notation, note, task).map(examify)
#let w17(..tasks) = [*WS17*#repr(tasks)]
#let w19(..tasks) = [*WS19*#repr(tasks)]
#let s20(..tasks) = [*SS20*#repr(tasks)]
#let w21(..tasks) = [*WS21*#repr(tasks)]
#let s22(..tasks) = [*SS22*#repr(tasks)]
...
= Aufgaben
#task(name: "Chromatic Number, Planar Graph with girth 6", exams: w19(3))[
...
#task(name: "Fan Lemma", exams: s20(2))[
